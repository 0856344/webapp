<template>
  <div class="section">
    <form class="flex flex-col gap-4 mx-4 sm:mx-0 sm:mr-4" @submit.prevent="updateMember">
      ...
      <div
        class="grid items-baseline grid-flow-row gap-2 self-center grow sm:py-2 sm:h-8 sm:grid-cols-contact sm:w-full"
        v-if="!member.paidForBy">
        <label class="block text-xs font-bold uppercase sm:text-end" for="checkbox"> Rechnungsadresse </label>
      ...
      <TextInput
        v-if="member.hasBillingAddress"
        :label="$t('firstName')"
        v-model="member.billingFirstName"
        :disabled="true"
        :isContact="true" />
      <TextInput
        v-if="member.hasBillingAddress"
        :label="$t('lastName')"
        v-model="member.billingLastName"
        :disabled="true"
        :isContact="true" />
      <TextInput
        v-if="member.hasBillingAddress"
        :label="$t('address')"
        v-model="member.billingAddress"
        :isContact="true" />
      <TextInput v-if="member.hasBillingAddress" :label="$t('zipCode')" v-model="member.billingZip" :isContact="true" />
      ...
      <div
        ...
        <select
          id="country-input"
          v-model="member.billingCountryCode"
          class="px-2 border-2 rounded-sm w-full m-auto sm:min-w-fit">
          <option v-for="country in countries" :value="country.id" v-bind:key="country.id">
            {{ country.name }}
          </option>
    ...
    <form
      ...
      <div v-if="!ibanIsValid && paymentMethod.iban" class="text-xs text-orange">IBAN ist ungültig</div>
      ...
      <div class="grid items-baseline grid-flow-row gap-2 grow sm:py-2 sm:grid-cols-contact">
        <label class="block text-xs font-bold uppercase sm:text-end" for="checkbox"> SEPA MANDAT </label>
        <div class="flex items-start gap-2">
          <input id="checkbox-sepa" class="w-4 h-4 ml-px" type="checkbox" :checked="sepaMandat" v-model="sepaMandat" />
          <div class="text-xs">
            Ich ermächtige die CAP.future GMBH, Zahlungen von meinem Konto mittels SEPA-Lastschrift einzuziehen.
            Zugleich weise ich mein Kreditinstitut an, die von der CAP.Future GMBH auf mein Konto gezogenen
      ...
      <div class="flex justify-center w-full sm:justify-end">
        <div v-if="loadingPayment">Saving…</div>
        <div v-else class="flex w-5/6 gap-2 mt-6 sm:w-auto">
          ...
          <button
            ...
            class="py-2 text-white rounded-sm grow sm:px-12 bg-orange ring-2 ring-orange-300 cursor:pointer disabled:bg-gray-700 disabled:ring-gray-300 disabled:cursor-default hover:bg-gray-900 hover:ring-gray-300"
            :disabled="
              !sepaMandat || !ibanIsValid || paymentMethod.iban === currentIban || paymentMethod.accountOwnerName === ''
            ">
            <font-awesome-icon icon="save" />
            {{ $t('save') }}
          </button>
  ...
  export default {
        ...
        get() {
          if (!this.ibanFieldFocus && this.paymentMethod.iban.length > 3) {
            return (
              ...
              this.paymentMethod.iban.substr(this.paymentMethod.iban.length - 4, 4)
            )
          } else {
        ...
        if (this.member) {
          if (
            ...
            if (!this.member.hasBillingAddress) {
              ...
              if (
                this.member.billingFirstName &&
                this.member.billingLastName &&
                this.member.billingAddress &&
                this.member.billingCity &&
                this.member.billingZip &&
                this.member.billingCountryCode
              ) {
                return true
              } else {
                return false
    ...
    methods: {
      ...
      async updatePaymentMethod(event) {
        ...
        this.$store
          .dispatch('updatePaymentMethod', Object.assign({}, updatePaymentRequest))
          .then(() => {
