      ...
      <div class="tags" :class="tagsCollapsed ? 'collapsed' : ''">
        ...
        <div class="tag-list">
          <div v-for="t in tags" :key="t.key" class="tag">
            <checkbox v-model="t.value" class="tag" theme="white">{{ t.name }}</checkbox>
          </div>
        </div>
      </div>
      <div class="search">
      ...
      <div v-if="machines && machines.length > 0" class="machine-list">
        <transition-group name="list">
          <machine-list-item
            ...
            :blok="item"
            :key="item.id"
            :state="getMachineState(item.content.machine_status_items)"
            class="list-item"></machine-list-item>
        </transition-group>
      </div>
...
<script>
  ...
  export default {
    ...
    async mounted() {
      this.states = await this.$store.dispatch('getMachineStates')
    },
    watch: {
      search() {
        this.update()
    ...
    computed: {
      ...
      multiMachineStates() {
        const filtered = Object.entries(this.machines).filter(([key, value]) => {
          const states = value.content.machine_status_items
          return Array.isArray(states) && states.length > 1 && states.some(s => this.states[s.fabmanId] === 'locked')
        })
