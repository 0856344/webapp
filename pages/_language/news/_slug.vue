  ...
  <div class="body">
    <div v-if="item">
      <div class="header">
        <div
          class="header-image"
          :style="{ 'background-image': 'url(' + item.content.image + ')' }"
        ></div>
      ...
      <div class="blogFeed-detail">
        <div class="left-content">
          <div class="social-media-link">
            <a
              v-if="item.content.link.url != ''"
              :href="item.content.link.url"
              class="info-block"
              ><img
                v-if="item.content.source.length != 0"
                class="source-img"
                :src="`/icons/${item.content.source}.png`"
            /></a>
      ...
      <div class="teaser">
        <component
          v-for="i in item.content.contentBloks"
          :blok="i"
          :is="i.component"
          :key="i.uid"
        />
      ...
      <div class="machine" v-if="machine != null && machine.length != 0">
        ...
        <div class="display-machines">
          <machine-preview v-for="m in machine" :id="m" :key="m" />
        </div>
      </div>
      <div
        class="maker"
      ...
      >
        ...
        <div class="display-makers">
          <maker-preview v-for="p in person" :id="p" :key="p" />
          <maker-preview v-for="p in member" :id="p" :key="p._uid" />
        </div>
      </div>
    </div>
...
export default {
  ...
  asyncData(context) {
    return context.store
      .dispatch("loadNewsItem", context.route.params.slug)
      .then((data) => {
