    ...
    <div class="tab-section">
      <div class="tab-section-menu">
        ...
        <MenuLink to="/me/trainings" icon="graduation-cap" style="color: white !important">
          <font-awesome-icon :style="{ color: '#E69140' }" v-if="!hasCompletedRequiredCourses" icon="info-circle" />
          {{ $t('trainings') }}
        </MenuLink>
        <MenuLink to="/de/gutscheine?action=redeem" icon="money-bill">Gutscheine einlösen</MenuLink>
        <MenuLink :isActive="$route.name.includes('invoices')" to="/me/invoices" icon="file-invoice"
          >{{ $t('invoices') }}
        </MenuLink>
      </div>
