<template>
  <div
    ...
    v-if="
      !blok.conditional ||
      (blok.conditional &&
        blok.condition === 'authenticated' &&
        !!this.$store.state.auth) ||
      (blok.conditional &&
        blok.condition == 'notAuthenticated' &&
        !!!this.$store.state.auth)
    "
  >
    <div
      class="w-full flex-col lg:col-start-2 lg:col-span-4"
      :class="[
        {
          'flex gap-2 lg:gap-8 rounded-xl px-4 lg:px-0 max-w-prose lg:max-w-none':
            hasProse,
        },
        { 'lg:flex-row': !hasTextAbove },
      ]"
    >
      <div
        v-if="hasProse"
        class="basis-1/2"
        :class="[{ 'lg:order-last': hasTextRight }]"
      >
