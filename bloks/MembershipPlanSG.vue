<template>
  <div
    ...
    :class="[
      {
        'lg:shadow-md lg:shadow-blue-700 lg:scale-105 hover:cursor-pointer': selected,
      },
    ...
    <div
      ...
      :class="{ 'bg-yellow': !!blok.inverted }">
      <h2
        class="uppercase transition-transform duration-300"
        :class="[{ 'group-hover:scale-110': !selected }, { 'text-gray-900': !!blok.inverted }]">
        {{ blok.name }}
      </h2>
      <!---div
    ...
    <template v-if="!updatedPricing">
      <div
        ...
        <div v-if="!isMobile">
          <span v-if="item == 'Coming Soon'" class="text-xl text-gray-400 transition duration-300">{{
            item.toUpperCase()
          }}</span>
          <font-awesome-icon v-else-if="item != ''" :icon="['fas', 'check']" class="text-3xl" />
          <span v-else class="text-6xl text-gray-300 transition duration-300">×</span>
        </div>
        <div v-else class="flex flex-col items-center">
          <span v-if="item.startsWith(comingSoonPrefix)" class="px-2 w-max font-bold bg-yellow rounded-md text-gray-900"
            >Coming Soon</span
          >
    ...
    <div
      ...
      :class="[{ 'bg-white': selected }, { 'bg-gray-100 group-hover:bg-white': !selected }]">
      <span v-if="blok.credits === '0'" class="text-lg transition duration-300">{{ $t('payPerUse') }}</span>
      <span v-else-if="blok.credits">
        <span v-if="!updatedPricing" class="text-xl font-bold">{{ blok.credits }} Credits</span>
        <span v-else class="text-xl font-bold">{{ blok.credits }} €</span>
        <span v-if="isMobile"><br />{{ $t('creditsDescription') }}</span>
      </span>
    </div>
    <template v-if="updatedPricing">
      <div
        class="inline-flex items-center justify-center px-2 py-1 text-sm text-center border-b-2 border-x-2 hyphens-auto transition duration-300 border-x-gray-900 border-b-gray-900"
        :class="[{ 'bg-white': selected }, { 'bg-gray-100 group-hover:bg-white': !selected }, ]">
        <span v-if="!isMobile" class="text-6xl text-gray-900 transition duration-300"
          ><font-awesome-icon :icon="['fas', 'check']" class="text-3xl" /></span
        >
        <span v-else>
          {{ $t('memberBenefits') }}
        </span>
      ...
      <div
        class="inline-flex items-center justify-center px-2 py-1 text-sm text-center border-b-2 border-x-2 hyphens-auto transition duration-300 border-x-gray-900 border-b-gray-900"
        :class="[{ 'bg-white': selected }, { 'bg-gray-100 group-hover:bg-white': !selected }, {'hidden': parseInt(blok.discount) <= 0 && isMobile}]">
        <span v-if="parseInt(blok.discount) <= 0 && !isMobile" class="text-6xl text-gray-300 transition duration-300"
          >×</span
        >
        <span v-else>
          <span class="text-xl font-bold">{{ blok.discount }}%</span>
          <span v-if="isMobile"><br />{{ $t('discount') }}</span>
        </span>
      </div>
      <div
        class="inline-flex items-center justify-center px-2 py-1 text-sm text-center border-b-2 border-x-2 hyphens-auto transition duration-300 border-x-gray-900 border-b-gray-900"
        :class="[{ 'bg-white': selected }, { 'bg-gray-100 group-hover:bg-white': !selected }, {'hidden': parseInt(blok.reservation_hours) <= 0 && isMobile}]">
        <span
          v-if="parseInt(blok.reservation_hours) <= 0 && !isMobile"
          class="text-6xl text-gray-300 transition duration-300"
          >×</span
        >
        <span v-else>
          <span class="text-xl font-bold">{{ blok.reservation_hours }} h / Woche</span>
          <span v-if="isMobile"><br />{{ $t('bookingHoursDescription') }}</span>
        </span>
      </div>
      <div
        class="inline-flex items-center justify-center px-2 py-1 text-sm text-center border-b-2 border-x-2 hyphens-auto transition duration-300 border-x-gray-900 border-b-gray-900"
        :class="[{ 'bg-white': selected }, { 'bg-gray-100 group-hover:bg-white': !selected }, {'hidden': blok.name === 'Flex' && isMobile}]">
        <span
          v-if="blok.name === 'Flex' && !isMobile"
          class="text-6xl text-gray-300 transition duration-300"
          >×</span
        >
        <span v-else>

          <span v-if="isMobile">{{ $t('memberGroups') }}</span>
          <font-awesome-icon v-else :icon="['fas', 'check']" class="text-3xl" />
        </span>
      </div>
      <div
        class="inline-flex items-center justify-center px-2 py-1 text-sm text-center border-b-2 border-x-2 hyphens-auto transition duration-300 border-x-gray-900 border-b-gray-900"
        ...
        <span
          v-if="!!!blok.yearly_charge && !isMobile"
          class="text-6xl text-gray-300 transition duration-300"
          >×</span
        >
        <span v-else-if="!!!blok.yearly_charge && isMobile">{{ $t('noYearlyCharge') }}</span>
        <span v-else>
          <span class="text-xl font-bold">{{ blok.price }} €</span>
          <span v-if="isMobile"><br />{{ $t('yearlyCharge') }}</span>
        </span>
    ...
    <div
      ...
      :class="[
        { 'bg-orange text-white': selected && !!!blok.inverted },
        { 'bg-yellow': !selected && !!!blok.inverted },
        { 'bg-gray-900 text-yellow': !!blok.inverted },
      ]">
      ...
      <h2
        ...
        {{ `${!!blok.yearly_charge ? 0 : blok.price},-` }}
      </h2>
...
<script>
  export default {
    props: ['blok', 'strings', 'isMobile', 'comingSoon', 'updatedPricing'],
    mounted() {
    ...
    computed: {
      ...
      features() {
        const features = Object.entries(this.strings).map(([key, value]) => {
          if (this.blok.features.includes(value) && this.comingSoon.includes(key)) {
            if (!this.isMobile) {
