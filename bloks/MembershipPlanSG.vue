  ...
  <div v-editable="blok" :id="this.$vnode.key"
    ...
    <div
      ...
      <div
        ...
        :class="{ 'group-hover:-rotate-12 ': !selected }">{{ `${blok.age_limit}+`
        }}</div>
        </div>
    <div v-for="item in features" :key="item?._uid"
      ...
      <div v-if="!isMobile">
        <span v-if="item == 'Coming Soon'"
          class="text-xl text-gray-400 transition duration-300">{{ item.toUpperCase() }}</span>
      ...
      <div v-else class="flex flex-col items-center">
        <span v-if="item.startsWith(this.comingSoonPrefix)" class="px-2 w-max font-bold  bg-yellow rounded-md text-gray-900">Coming Soon</span>
        <span>{{ item.replace(this.comingSoonPrefix,'') }}</span>        
      </div>
    </div>
    <div
