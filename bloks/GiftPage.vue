    ...
    <div class="gift-card-body">
      <div class="gift-card-header">
        <h2 v-if="action" class="gift-page-headline">
          ...
          <span class="svg-h2"> {{ $t('giftCard') }} {{ action === 'buy' ? 'kaufen' : 'einlösen' }} </span>
          ...
          <section class="display-item">
            <div class="top">
              <div
                class="top-image"
                :style="{
                  'background-image': 'url(' + $resizeImage(blok.imageRedeem, '1500x1500') + ')',
                }"></div>
            </div>
            <div class="bottom">
              <div class="bottom-text">
              ...
              <div class="buy-redeem-button" @click="goToRedeem()">
                {{ $t('redeem') }}
              </div>
            </div>
          </section>
        ...
        <template>
          <template v-if="action === 'redeem'">
            <div v-if="step === 0" class="description-gift-card">
              ...
              <br />
              <p style="text-align: justify"><b>Option 1: Mitgliedskonto</b></p>
              <p style="text-align: justify; margin-bottom: 0px">
                Du kannst deinen Wertgutschein in voller Höhe oder auch einen Teilbetrag auf dein Mitgliedskonto
                aufladen und davon deine Mitgliedschaft, Maschinennutzung, verwendete Materialien aber auch Getränke
                bezahlen. Klingt gut? Dann einfach ins Feld „Gutschein einlösen“ den Gutscheincode eintragen und auf
                „Einlösen“ klicken. Dein Guthaben kannst du in deinem
                <router-link v-if="isAuthenticated" to="/me">Mitgliedskonto</router-link>
                <a v-if="!isAuthenticated" @click="login()" target="_self" href="javascript:;">Mitgliedskonto</a>
                unter
                ...
                <div class="card">
                  ...
                  <div class="login-reminder">
                    <div>
                      ...
                      Bitte
                      <a @click="login()" target="_self" href="javascript:;">logge dich ein</a>, um deinen Gutschein
                      einlösen zu könnnen!
                    </div>
                    <br />
                  </div>
                  <div class="buttons">
                    <button class="input-button-primary" @click="goToGutscheine()">
                      {{ $t('back') }}
                    </button>
                  </div>
                </div>
              ...
              <div v-if="isAuthenticated && !isGiftcardSelected">
                <div class="card">
                  <div class="input-redeem-card">
                    <span class="span"> Gutschein einlösen</span>
                    <div class="redeem-card-bottom">
                      <TextInput v-model="giftcardCode" label="Gutscheincode:"></TextInput>
                      <div class="image">
                  ...
                  <div class="buttons">
                    <button class="input-button-payment" @click="goToGutscheine()">
                      {{ $t('back') }}
                    </button>
                    <button class="input-button-payment" :disabled="!giftcardCode" @click="getGiftCard">Weiter</button>
                  </div>
                </div>
              </div>
              <div v-if="isAuthenticated && isGiftcardSelected">
                <div class="card">
                  <div class="input-redeem-card">
                    <span class="span"> Gutschein einlösen</span>
                    <span class="code-span" style="margin-bottom: 10px; top: 10px"
                      >aktuelles Guthaben:
                      <div style="font-weight: bold; color: darkgreen">{{ deposit | replaceDotByComma }}€</div></span
                    >
                    <div class="redeem-card-bottom">
                      <div class="code">
                        ...
                        <input
                          ...
                          @input="checkValue(redeemDeposit)"
                  ...
                  <div class="buttons">
                    ...
                    <button
                      v-if="!isGiftcardSelected"
                      class="input-button-payment"
                      :disabled="!giftcardCode"
                      @click="getGiftCard"></button>
                    <button
                      v-if="isGiftcardSelected"
                      class="input-button-payment"
              ...
              <br />
              <p><b>Option 2: Workshop & Events</b></p>
              <p style="text-align: justify">
                Alternativ kannst du deinen Wertgutschein auch für einen unserer Workshop & Events verwenden. Suche dir
                dazu zunächst den passenden Termin aus unserem aktuellen
                <router-link to="/de/workshops">{{ 'Angebot' }}</router-link>
                aus und lege den Kurs in deinen Warenkorb. Deinen Wertgutschein kannst du später im Bestellvorgang bei
                den Zahlungsmethoden einlösen. Ein etwaiger Restbetrag kann mit demselben Gutscheincode zu einem
                späteren Zeitpunkt eingelöst werden.
              </p>
            </div>
          </template>
        </template>
  ...
  export default {
    ...
    methods: {
      ...
      async redeem() {
        ...
        payload = { ...payload, ...captchaData }
        await this.$store
          .dispatch('redeemGiftCard', payload)
          .then(response => {
            //console.log('success', response)
            this.$toast.show('Der Gutschein wurde erfolgreich eingelöst!', {
              className: 'goodToast',
